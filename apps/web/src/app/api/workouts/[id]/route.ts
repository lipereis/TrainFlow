...
export async function GET(_req: NextRequest, ctx: Ctx) {
  return withHandler(async () => {
    ...
    return jsonOk(await workoutsService.get(trainerId, (await ctx.params).id));
  });
}
...
export async function PATCH(req: NextRequest, ctx: Ctx) {
  return withHandler(async () => {
    ...
    return jsonOk(await workoutsService.update(trainerId, (await ctx.params).id, body));
  });
}
...
export async function DELETE(_req: NextRequest, ctx: Ctx) {
  return withHandler(async () => {
    const { trainerId } = await requireTrainerId();
    await workoutsService.remove(trainerId, (await ctx.params).id);
