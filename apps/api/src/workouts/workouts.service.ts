...
export class WorkoutsService {
  ...
  async remove(trainerId: string, programId: string): Promise<void> {
    await this.requireOwnedProgram(trainerId, programId);
    await this.prisma.workoutProgram.delete({ where: { id: programId } });
  }

  async addDay(trainerId: string, programId: string, input: WorkoutDayInput) {
    await this.requireOwnedProgram(trainerId, programId);
    const existing = await this.prisma.workoutDay.findMany({
      where: { programId },
    ...
    const row = await this.prisma.workoutDay.create({
      data: {
        programId,
  ...
  async updateDay(
    ...
    await this.requireDayInProgram(trainerId, programId, dayId);
    const row = await this.prisma.workoutDay.update({
      where: { id: dayId },
      data: {
        ...(input.name !== undefined ? { name: input.name } : {}),
  ...
  async reorderDays(
    ...
    if (
      ...
      throw new NotFoundException({
        code: "DAY_NOT_FOUND",
        message: "Reorder ids must match all days in the program",
      });
    }
  ...
  async addExercise(
    ...
    const nextOrder =
      input.sortOrder ??
      (existing[0] ? existing[0].sortOrder + 1 : 0);
    const row = await this.prisma.workoutExercise.create({
      data: {
        dayId,
  ...
  async reorderExercises(
    ...
    if (
      ...
      throw new NotFoundException({
        code: "WORKOUT_EXERCISE_NOT_FOUND",
        message: "Reorder ids must match all exercises in the day",
      });
    }
    await this.prisma.$transaction(
      orderedIds.map((id, index) =>
